    ...
    reducers: {
        ...
        addCustomizableItem: (
            ...
            if (existingRestaurantCart) {
                ...
                if (existingItem) {
                    const existingCustomizationIndex = existingItem?.customizations?.findIndex((cust: any) =>
        ...
        updateCustomizableItem: (
            ...
            if(matchingCustomizationIndex !== -1){
                ...
                targetCustomization.qunatity = newCustomization.quantity;
            ...
            item.quantity = item?.customizations?.reduce(
                (sum, cust) => sum + cust.quantity,0,
            );

            item.cartPrice = item?.customizations?.reduce(
                (sum, cust) => sum + cust.cartPrice,0,
            );
